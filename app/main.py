...
6 Tabs: Summarize | Extract | Q&A | Analytics | Live Monitor | Power BI Dashboard
Made by Ujjwal Kaushik
Fixed: API key hidden, model updated to gemini-2.5-flash
"""
...
st.set_page_config(page_title="Web Scraping AI Bot",
...
<style>
  ...
  * { font-family: 'Inter', sans-serif; }
  ...
  .main-header p { margin:.3rem 0 0; opacity:.7; font-size:.9rem; color:#a78bfa; }
  ...
  .kpi-label { font-size: .75rem; color: #7c7ca0; margin-top: 4px; text-transform: uppercase; letter-spacing: .05em; }
  .kpi-sub { font-size: .72rem; color: #a78bfa; margin-top: 3px; }

  /* ── Metric cards (change indicators) ── */
  .metric-up   { color: #34d399 !important; font-weight: 700; }
  .metric-down { color: #f87171 !important; font-weight: 700; }
  .metric-neu  { color: #94a3b8 !important; }
  ...
  .result-card {
    ...
    padding: 1rem 1.2rem; border-radius: 10px; margin: .8rem 0;
    color: #e2e8f0;
  }
  ...
  .stButton > button {
    ...
    border-radius: 10px !important; padding: .65rem 1.2rem !important;
  ...
  [data-testid="stSidebar"] {
    background: linear-gradient(180deg, #0d0d1a 0%, #13102a 100%) !important;
  ...
  .stTabs [data-baseweb="tab"] {
    border-radius: 10px !important; color: #7c7ca0 !important;
  ...
  .live-banner {
    ...
    border-radius: 10px; padding: .6rem 1rem; margin-bottom: .8rem;
  ...
  .chart-wrap {
    ...
    border-radius: 14px; padding: .5rem; margin-bottom: .8rem;
  ...
  [data-testid="stMetricLabel"] { color: #7c7ca0 !important; font-size:.8rem !important; }
</style>
...
<div class="main-header">
  <h1>🤖 Web Scraping AI Bot</h1>
  <p>Gemini 2.5 Flash · Live Monitor · Power BI Dashboard · Made by Ujjwal Kaushik</p>
</div>
""", unsafe_allow_html=True)

# ── Session state ─────────────────────────────────────────────────────────────
for k, v in [("monitor_session",None),("monitor_running",False),
              ("dashboard_df",None),("monitor_url",""),("last_charts",[])]:
...
with st.sidebar:
    ...
    # API Key - stored securely, never shown
    ...
    if existing_key:
        ...
        <span style='color:#34d399;font-size:.85rem;'>
        ✅ Gemini API Key loaded</span><br>
        <span style='color:#7c7ca0;font-size:.75rem;'>
        {existing_key[:4]}{'*'*20}{existing_key[-4:] if len(existing_key)>8 else '****'}
        </span></div>""", unsafe_allow_html=True)
    else:
        new_key = st.text_input("Gemini API Key", type="password",
            placeholder="Enter your API key",
            help="Your key is never displayed. Get one at aistudio.google.com")
        if new_key:
    ...
    st.markdown("---")
    st.markdown("""
    <div style='text-align:center;color:#4a4a6a;font-size:.75rem;'>
    Made by <b style='color:#a78bfa;'>Ujjwal Kaushik</b>
    </div>""", unsafe_allow_html=True)
...
def _validate(url, need_api=True):
    ...
    if need_api and not os.getenv("GEMINI_API_KEY") and not use_local:
        st.error("❌ Gemini API Key required. Add it in the sidebar."); return False
    return True
...
tab1,tab2,tab3,tab4,tab5,tab6 = st.tabs([
    "📝 Summarize","📊 Extract Data","💬 Ask Question",
    "🔬 Analytics","📡 Live Monitor","📈 Dashboard",
])
...
# ════════════════════════════════════════════════════════════════════
with tab1:
    _section("📝 Summarize Any Web Page")
    ...
    if st.button("🚀 Summarize Page", key="btn_s"):
        if _validate(url_s):
            with st.spinner("Scraping and summarizing..."):
                try:
                    r = _get_pipeline().run_summarize(url_s, style=style)
                    if r.get("error"): st.error(f"❌ {r['summary']}")
                    else:
                        meta = r.get("metadata",{})
                        if meta.get("title"):
                            st.markdown(f"<h3 style='color:#a78bfa;'>📄 {meta['title']}</h3>",
                                unsafe_allow_html=True)
                        _section("🤖 AI Summary")
                        st.markdown(f'<div class="result-card">{r["summary"]}</div>',
                            unsafe_allow_html=True)
                        c1,c2,c3 = st.columns(3)
                        c1.metric("📏 Length", f"{r.get('content_length',0):,} chars")
                        c2.metric("🎨 Style", style.title())
                        c3.metric("🌐 Source", url_s[:20]+"...")
                        if any(meta.values()):
                            with st.expander("📋 Page Metadata"):
                                for k,v in meta.items():
                                    if v: st.markdown(f"**{k.replace('_',' ').title()}:** {v}")
                        st.download_button("⬇️ Download Summary",data=r["summary"],
...
with tab2:
    ...
    mode_text  = "🔧 Local Mode — HTML extraction, no API needed" if use_local else "🤖 AI Mode — Gemini 2.5 Flash extraction"
    ...
    color:{mode_color};font-size:.9rem;'>{mode_text}</div>""",
    unsafe_allow_html=True)
                ...
                try:
                    ...
                    if records:
                        ...
                        if not df.empty:
                            ...
                            _save_dashboard(df)
                            st.info("💡 Data saved → go to **📈 Dashboard** tab to visualize!")
                        d1,d2 = st.columns(2)
                        with d1:
                        ...
                        st.warning("⚠️ No records extracted. Try Local Mode or different Content Type.")
                except Exception as e: st.error(f"❌ {e}"); logger.exception(e)

# ════════════════════════════════════════════════════════════════════
# TAB 3 — Q&A
# ════════════════════════════════════════════════════════════════════
with tab3:
    _section("💬 Ask Any Question About Any Page")
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    url_q    = st.text_input("Page URL", key="url_q")
    question = st.text_area("Your Question", height=90, key="question",
        placeholder="What is the price? Who is the author? What are the key facts?")
    st.markdown('</div>', unsafe_allow_html=True)

    if st.button("💬 Ask Gemini", key="btn_q"):
        if _validate(url_q):
            if not question.strip(): st.warning("⚠️ Enter a question.")
            else:
                with st.spinner("Asking Gemini 2.5 Flash..."):
                    try:
                        r = _get_pipeline().run_qa(url_q, question.strip())
                        st.markdown(f"""
                        <div style='background:rgba(96,165,250,.08);border:1px solid
                        rgba(96,165,250,.2);border-radius:10px;padding:.8rem 1rem;
                        color:#93c5fd;margin:.5rem 0;'>❓ {r['question']}</div>""",
                        unsafe_allow_html=True)
                        _section("🤖 Gemini's Answer")
                        st.markdown(f'<div class="result-card">{r["answer"]}</div>',
                            unsafe_allow_html=True)
    ...
    if inp=="🌐 Scrape URL":
            ...
            if _validate(url_a):
                with st.spinner("Running autonomous analysis..."):
                    try:
                        ...
                        if not content: st.error("❌ Failed to fetch."); st.stop()

                        prog.progress(25, "🔍 Extracting data...")
                        html = pipeline._fetch_html(url_a)
                        records = []
                        if html:
                            from scraper.local_extractor import smart_local_extract
                            from pipeline.deduplicator import clean_records
                        ...
                        if not records and os.getenv("GEMINI_API_KEY"):
                        ...
                        if df_a is not None and not df_a.empty: _save_dashboard(df_a)

                        prog.progress(55, "🧠 Analyzing...")
                        ...
                        prog.progress(90, "📊 Rendering results...")
                                ...
                                for i in range(0,len(df_res["charts"]),2):
                                    ...
                                    for j,col in enumerate(cols):
                                        if i+j<len(df_res["charts"]):
                                            with col:
                                                _chart_wrap(df_res["charts"][i+j]["figure"],
                                                    f"ac_{i}_{j}")
                        ...
                        if text_res:
                            ...
                            for ch in text_res.get("charts",[]):
                                _chart_wrap(ch["figure"],f"tc_{ch['title']}")
        ...
        if uploaded:
            try:
                ...
                st.success(f"✅ Loaded {len(df_up)} rows × {len(df_up.columns)} cols")
                st.dataframe(df_up.head(5),use_container_width=True)
                _save_dashboard(df_up)
                st.info("💡 Go to **📈 Dashboard** tab to visualize!")
                if st.button("🚀 Analyze CSV",key="btn_csv"):
                    if not os.getenv("GEMINI_API_KEY"): st.error("❌ API Key required.")
                    else:
                        with st.spinner("Analyzing..."):
                            try:
                                from analytics.agent import DataAnalyticsAgent
                                r = DataAnalyticsAgent().analyze_dataframe(df_up,content_type="uploaded CSV")
                                st.success("✅ Done!")
                                for i in range(0,len(r.get("charts",[])),2):
                                    cols=st.columns(2)
                                    for j,col in enumerate(cols):
                                        if i+j<len(r["charts"]):
                                            with col: _chart_wrap(r["charts"][i+j]["figure"],f"cc_{i}_{j}")
...
with tab5:
    ...
    <div class="live-banner">
      ...
      <span style='color:#7c7ca0;font-size:.85rem;'>
        Scrapes every 60s · Previous vs Current · 🟢↑ / 🔴↓ change indicators · Alerts
      </span>
    </div>""", unsafe_allow_html=True)
    ...
    if st.session_state.monitor_running and st.session_state.monitor_session:
        ...
        <div class="live-banner">
          ...
          <span style='color:#7c7ca0;font-size:.85rem;'>
            Snapshot #{len(session.snapshots)+1} · Every 60s
          </span>
        </div>""", unsafe_allow_html=True)
            ...
            try:
                ...
                if not records:
                    ...
                    if changes["has_changes"]:
                        ...
                        with cc1:
                            if changes["added"]:
                                st.markdown(f"""<div style='background:rgba(52,211,153,.1);
                                border:1px solid rgba(52,211,153,.3);border-radius:12px;
                                padding:1rem;text-align:center;'>
                                <div style='color:#34d399;font-size:2rem;font-weight:700;'>
                                +{len(changes["added"])}</div>
                                <div style='color:#7c7ca0;font-size:.8rem;'>New Records</div>
                                </div>""", unsafe_allow_html=True)
                        with cc2:
                            if changes["removed"]:
                                st.markdown(f"""<div style='background:rgba(248,113,113,.1);
                                border:1px solid rgba(248,113,113,.3);border-radius:12px;
                                padding:1rem;text-align:center;'>
                                <div style='color:#f87171;font-size:2rem;font-weight:700;'>
                                -{len(changes["removed"])}</div>
                                <div style='color:#7c7ca0;font-size:.8rem;'>Removed</div>
                                </div>""", unsafe_allow_html=True)
                        with cc3:
                            if changes["changed"]:
                                st.markdown(f"""<div style='background:rgba(96,165,250,.1);
                                border:1px solid rgba(96,165,250,.3);border-radius:12px;
                                padding:1rem;text-align:center;'>
                                <div style='color:#60a5fa;font-size:2rem;font-weight:700;'>
                                ~{len(changes["changed"])}</div>
                                <div style='color:#7c7ca0;font-size:.8rem;'>Changed</div>
                                </div>""", unsafe_allow_html=True)
                    ...
                    _section("📋 Previous vs Current Snapshot")
                    pt,ct2=st.columns(2)
                    with pt:
                        st.markdown(f"<div style='color:#7c7ca0;font-size:.85rem;margin-bottom:.3rem;'>⏮ Previous ({('—' if not prev else prev['timestamp'][11:19])})</div>",unsafe_allow_html=True)
                    ...
                    with ct2:
                        st.markdown(f"<div style='color:#34d399;font-size:.85rem;margin-bottom:.3rem;'>▶ Current ({curr['timestamp'][11:19]})</div>",unsafe_allow_html=True)
                        st.dataframe(pd.DataFrame(records).head(8),use_container_width=True)

                    if len(session.snapshots)>=2:
                        import plotly.graph_objects as go
                        hist_df=pd.DataFrame([{"Time":s["timestamp"][11:19],
                            "Records":s["record_count"],"Changes":s["changes"]["total_changes"]}
                            for s in session.snapshots])
                        fig_h=go.Figure()
                        fig_h.add_trace(go.Scatter(x=hist_df["Time"],y=hist_df["Records"],
                            name="Records",line=dict(color="#a78bfa",width=2),
                            fill="tozeroy",fillcolor="rgba(167,139,250,.1)"))
                        fig_h.add_trace(go.Bar(x=hist_df["Time"],y=hist_df["Changes"],
                            name="Changes",marker_color="#f87171",opacity=.6,yaxis="y2"))
                        fig_h.update_layout(
                            paper_bgcolor="rgba(13,13,26,0.8)",plot_bgcolor="rgba(13,13,26,0)",
                            font=dict(color="#c4b5fd"),height=250,
                            margin=dict(l=40,r=40,t=20,b=40),
                            legend=dict(bgcolor="rgba(0,0,0,0)"),
                            yaxis=dict(gridcolor="rgba(139,92,246,.15)"),
                            yaxis2=dict(overlaying="y",side="right",gridcolor="rgba(0,0,0,0)"),
                        )
                        _chart_wrap(fig_h,"hist_ch")

                    if session.alert_history:
                        _section("🚨 Alerts Triggered")
                        for al in reversed(session.alert_history[-8:]):
                            ...
                            {pct} · {al['triggered_at'][11:19]}</div>""",
                            unsafe_allow_html=True)

                    _save_dashboard(pd.DataFrame(records))
                    st.info("💡 Current data saved → **📈 Dashboard** tab")

            except Exception as e: st.error(f"❌ {e}"); logger.exception(e)

        st.markdown("""<div style='text-align:center;color:#4a4a6a;font-size:.8rem;
        margin-top:.8rem;padding:.5rem;background:rgba(255,255,255,.02);
        border-radius:8px;border:1px solid rgba(255,255,255,.05);'>
        ⏱️ Auto-refreshes every 60s · Click <b>Start Monitoring</b> to refresh now
        </div>""", unsafe_allow_html=True)
        time.sleep(1)
        st.rerun()
...
# ════════════════════════════════════════════════════════════════════
# TAB 6 — FULL POWER BI DASHBOARD
# ════════════════════════════════════════════════════════════════════
with tab6:
    st.markdown("""
    <div style='background:linear-gradient(135deg,rgba(124,58,237,.15),rgba(37,99,235,.1));
    border:1px solid rgba(124,58,237,.25);border-radius:14px;padding:1rem 1.5rem;
    margin-bottom:1rem;display:flex;align-items:center;gap:1rem;'>
      <span style='font-size:2rem;'>📈</span>
      <div>
        <div style='color:#a78bfa;font-size:1.2rem;font-weight:700;'>
          Power BI Style Dashboard
        </div>
        <div style='color:#4a4a6a;font-size:.85rem;'>
          All charts auto-generated · Dark CRM theme · Every chart type
        </div>
      </div>
    </div>""", unsafe_allow_html=True)
    ...
    if db_src.startswith("📊"):
        if st.session_state.dashboard_df is not None:
            ...
            <span style='color:#7c7ca0;'> Extract data from Tab 2 or 4 first, or upload CSV.</span>
            </div>""", unsafe_allow_html=True)
    else:
    ...
    if df_db is not None and not df_db.empty:
        if st.button("🚀 Generate Full Dashboard", key="btn_gen"):
            ...
            Click <b>Generate Full Dashboard</b> to auto-build all charts
